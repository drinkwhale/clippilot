...
from src.core.youtube.transcription_service import TranscriptionService
...
@router.post(
    ...
    response_model=TranscriptResponse,
    response_model_by_alias=True,
    summary="YouTube 영상 음성 자막 수집",
    description="YouTube 영상의 음성을 OpenAI Whisper API로 텍스트 자막으로 변환합니다.",
)
@limiter.limit("5/minute")
async def transcribe_video_audio(
    request: Request,
    video_id: str,
    language: Optional[str] = Query("ko", description="언어 코드 (ko, en, ja 등)"),
    current_user: dict = Depends(get_current_user),
    cache_service: CacheService = Depends(get_cache_service),
):
    """
    YouTube 영상 음성 자막 수집 API

    OpenAI Whisper API를 사용하여 YouTube 영상의 음성을 텍스트로 변환합니다.

    - **video_id**: YouTube 영상 ID (필수)
    - **language**: 언어 코드 (ko, en, ja 등, 기본값: ko)

    Rate Limit: 5 req/min (Whisper API 처리 시간 고려)
    Cache: 24시간 TTL
    """
    try:
        # 캐시 키 생성
        cache_key = f"youtube:transcribe:{video_id}:{language}"

        # 캐시 확인
        cached_result = cache_service.get(cache_key)
        if cached_result:
            logger.info(f"캐시된 자막 수집 결과 반환: video_id={video_id}")
            return cached_result

        # TranscriptionService로 음성 자막 수집
        transcription_service = TranscriptionService()
        result = transcription_service.transcribe_video(video_id, language)

        response = TranscriptResponse(
            video_id=result["video_id"],
            language=result["language"],
            segments=[TranscriptSegment(**seg) for seg in result["segments"]],
            full_text=result["text"],
        )

        # 캐시 저장 (24시간 TTL - 음성 자막은 변경되지 않으므로 길게 설정)
        cache_service.set(cache_key, response.model_dump(by_alias=True), ttl=86400)

        logger.info(
            f"음성 자막 수집 성공: video_id={video_id}, "
            f"segments={len(result['segments'])}, duration={result['duration']:.2f}s"
        )
        return response

    except Exception as e:
        logger.error(f"음성 자막 수집 실패: video_id={video_id}, error={str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"음성 자막 수집 중 오류가 발생했습니다: {str(e)}",
        )
