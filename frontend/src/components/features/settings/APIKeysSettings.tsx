/**
 * API Keys 관리 컴포넌트 (Settings 페이지용)
 * YouTube, OpenAI, Pexels 등 모든 API 키를 통합 관리
 */
...
import { useAPIKeys, useCreateAPIKey, useDeleteAPIKey } from "@/lib/hooks/useAPIKeys";
import type { ServiceName } from "@/lib/types/api-keys";
import { Loader2, Eye, EyeOff, Key, ExternalLink, Trash2, Check, X } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      {SERVICES.map((service) => {
        ...
        const apiKey = apiKeys?.find((k) => k.service_name === service.name);

        return (
          <div key={service.name} className="border rounded-lg p-4 space-y-4">
            {/* 서비스 헤더 */}
            ...
            {isEditing || !configured ? (
                ...
                <div className="flex gap-2">
                  ...
                  {configured && (
                    <Button
                      ...
                      onClick={() => {
                        setEditingService(null);
                        setKeyValues((prev) => ({ ...prev, [service.name]: "" }));
                      }}
                    >
                      취소
                    </Button>
              ...
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  마지막 사용: {apiKey?.last_used_at ? new Date(apiKey.last_used_at).toLocaleString("ko-KR") : "사용 기록 없음"}
                </div>
                <div className="flex gap-2">
                  <Button
                    size="sm"
      ...
      {process.env.NODE_ENV === "development" && (
        <Alert>
          <AlertDescription>
            <strong>개발 환경:</strong> API 키는 localStorage에 저장됩니다.
            프로덕션 환경에서는 Supabase에 암호화되어 저장됩니다.
          </AlertDescription>
        </Alert>
      )}
    </div>
